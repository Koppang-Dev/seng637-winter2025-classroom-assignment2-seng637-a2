...
| **Method Name**                                  | **Description**                                        | **Expected Behavior**                                  | **Edge Cases Covered**                                  |
|--------------------------------------------------|--------------------------------------------------------|--------------------------------------------------------|---------------------------------------------------------|
| `newRangeShouldBeNull`                          | Combines two `null` ranges                             | Returns `null`                                         | Both ranges are `null`                                  |
| `returnOtherRange`                              | Combines `null` with a valid range                    | Returns the valid range                                | One range is `null`                                    |
| `createRangeWithBothPositive`                   | Combines two valid ranges                             | Returns the correctly merged range                    | Overlapping and non-overlapping ranges                 |
| `constrainWithValueInsideRange`                 | Constrains value inside range                         | Returns the same value                                | Value within the range                                 |
| `constrainWithValueBelowLower`                  | Constrains value below lower bound                    | Returns lower bound                                   | Value below range                                      |
| `constrainWithValueAboveUpper`                  | Constrains value above upper bound                    | Returns upper bound                                   | Value above range                                      |
| `intersectsForOneBound`                         | Checks intersection when one bound overlaps           | Returns `true`                                        | Partial overlap                                        |
| `intersectsIsFalse`                             | Checks intersection when ranges do not overlap        | Returns `false`                                       | Non-overlapping ranges                                |
| `intersectsWithImproperRange`                   | Checks intersection with invalid range (`low > high`) | Throws `IllegalArgumentException`                     | Improper range input                                  |
| `intersectsRangeWithin`                         | Checks if a range is completely inside another        | Returns `true`                                        | Fully contained range                                 |
| `intersectsRangeOutside`                        | Checks if a larger range completely contains another  | Returns `true`                                        | Larger range encapsulating smaller one                |
| `expandWithValueLargerThanUpper`                | Expands range with a value above the upper bound     | Updates upper bound, lower bound remains the same     | Value greater than upper bound                        |
| `expandWithValueSmallerThanLower`               | Expands range with a value below the lower bound     | Updates lower bound, upper bound remains the same     | Value less than lower bound                           |
| `expandWithValueInsideRange`                    | Expands range with a value inside range              | No change in range                                    | Value inside the range                                |
| `GetLengthForPositiveValues`                    | Computes length for a range with positive values     | Returns the correct length                           | Standard positive range                               |
| `GetLengthForZeroLength`                        | Computes length for a zero-length range              | Returns `0.0`                                        | Upper and lower bounds are the same                  |
| `GetLengthForNegativeLowerBound`                | Computes length for a range crossing zero            | Returns correct positive length                      | Negative lower bound, positive upper bound           |
| `GetLengthForFullyNegativeBounds`               | Computes length for a fully negative range           | Returns correct positive length                      | Fully negative range                                 |
